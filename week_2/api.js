...
const values = Object.values(dataSet1);
const entries = Object.entries(dataSet1);

const comp1 = document.getElementById('comp1');

comp1.innerHTML = '<ul>' + entries.map(function (item) {  
  return `<li><span>keys:</span> ${item[0]} <span>values:</span> ${item[1]} </li>`;         
}).join('') + '</ul>'

const comp2 = document.getElementById('comp2');

comp2.innerHTML = '<ul>' + values.map(function (el) {
